...
class Expt:
    ...
    def showAnimation(self, addlTitle=""):
        ...
        axs['C'].set_xlim(0,self.tmax)
        axs['C'].set_ylim(0,1500)
        axs['C'].set_xlabel("Time")
        axs['C'].set_ylabel("Energy")
        axs['C'].set_title("Energy over time")
        
        #TODO: update all titles, etc.
        ...
        tvar = np.arange(0, self.tmax+self.dt, self.dt)
        historicE = np.zeros_like(tvar)
        historicKE = np.zeros_like(tvar)
        historicPE = np.zeros_like(tvar)
        Eline, = axs['C'].plot(tvar, historicE, 'r-', label="Average E")
        KEline, = axs['C'].plot(tvar, historicKE, 'b-', label="Kinetic E")
        PEline, = axs['C'].plot(tvar, historicPE, 'g-', label="Potential E")
        axs['C'].legend()
        ...
        def frame(_):
            ...
            historicE[int(.1+self.t/self.dt)] = self.avgE
            historicKE[int(.1+self.t/self.dt)] = self.avgKE
            historicPE[int(.1+self.t/self.dt)] = self.avgPE
            ...
            if self.updatectr % self.updateGraphsEvery == 0:
                ...
                #energy graphs
                Eline.set_data(tvar, historicE)
                KEline.set_data(tvar, historicKE)
                PEline.set_data(tvar, historicPE)
            ...
            self.updatectr += 1
            return points, bar_container.patches, title, Eline, KEline, PEline
